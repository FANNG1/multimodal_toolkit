...
def test_failed_run_report_does_not_require_lance(tmp_path):
    (tmp_path / "summary.json").write_text(
        json.dumps(
            {"run_id": "run-1", "state": "failed", "elapsed_s": 2.5, "error": "boom"}
        )
    )
        ...
        writer.writerow(
            {
                ...
                "ray_process_rss_bytes": 50,
                "cpu_percent": 10,
                "object_store_used_bytes": 20,
            }
        )
        writer.writerow(
            {
                ...
                "ray_process_rss_bytes": 75,
                "cpu_percent": 20,
                "object_store_used_bytes": 30,
            }
        )
...
def test_baseline_report_preserves_failed_repeat(tmp_path):
    reports = [
        {
            "run_id": "base-repeat-01",
            "state": "success",
        ...
        },
        {
            "run_id": "base-repeat-02",
            "state": "failed",
            "summary": {"elapsed_s": 45.0, "error": "timeout"},
            "resources": {"ray_process_rss_peak_bytes": 123},
        },
    ]
    result = build_baseline_report(
        tmp_path,
        reports,
        {"run_id": "base", "rows": 50, "duration_s": 60.0, "planned_repeats": 2},
    )
    assert result["all_succeeded"] is False
    assert result["passes_repeatability_slo"] is False
    assert result["throughput_cv"] is None
    assert result["repeat_results"][1]["error"] == "timeout"
    assert result["repeat_results"][1]["ray_process_rss_peak_bytes"] == 123
    assert "All succeeded: `NO`" in (tmp_path / "baseline-summary.md").read_text()
